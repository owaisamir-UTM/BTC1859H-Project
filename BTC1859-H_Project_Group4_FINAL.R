...
ggplot(data_bq, aes(
  x = factor(berlin_sleepiness_scale),
  ...
  labs(
    title = "Distribution of BSS Scores",
    x = "Category",
    y = "Count"
  ) +
...
boxplot_data <- data_sub %>%
  ...
  pivot_longer(
    cols = c(BSS_thresh, ESS_thresh, AIS_thresh),
    names_to = "Scale", # the name of the scale (BSS, AIS, ESS)
    values_to = "Threshold"
  ) # TRUE = above threshold, FALSE = below threshold
...
ggplot(data_clean, aes(x = Sleep_Score_F, fill = Sleep_Score_F)) +
  ...
  scale_fill_brewer(palette = "Set1") +
  labs(
    title = "Distribution of Sleep Disturbance Scores",
    x = "Sleep Disturbance Score",
